...
local user_opts = {
  ...
  ytdlpQuality =
  '-f bestvideo[vcodec^=avc][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best' -- what quality of video the download button uses (max quality mp4 by default)
}
            ...
            if element.thumbnailable and not thumbfast.disabled then
              ...
              if osd_w then
                ...
                if (mp.get_property_number("duration")) then
                  hover_sec = mp.get_property_number("duration") * sliderpos /
                      100
                end
...
function checktitle()
  ...
  local artist = mp.get_property("filtered-metadata/by-key/Artist") or
      mp.get_property("filtered-metadata/by-key/Album_Artist") or mp.get_property("filtered-metadata/by-key/Uploader")
  ...
  if user_opts.showyoutubecomments then
    ...
    local ret = mp.command_native_async({
      ...
      local filename = mp.command_native({ "expand-path", user_opts.commentsdownloadpath .. '/' }) ..
          mp.get_property("media-title") .. ".info.json"
      print(filename)
...
function exec_description(args, result)
  local ret = mp.command_native_async({
    ...
    if state.youtubeuploader then
      state.localDescriptionClick = state.localDescriptionClick:gsub("Uploaded by: <$\\N!uploader!\\N$>",
        "Uploaded by: " .. state.youtubeuploader)
    else
    ...
    if beforeLastPattern then
      ...
      if desc then
        ...
        afterLastPattern = afterLastPattern:gsub("Views:", emoticon.view):gsub("Comments:", emoticon.comment):gsub(
          "Likes:", emoticon.like):gsub("Dislikes:", emoticon.dislike) -- replace with icons
...
function osc_init()
  ...
  ne.visible = (osc_param.playresx >= 900 - outeroffset - (user_opts.showloop and 0 or 100) - (user_opts.showontop and 0 or 100) - (user_opts.showinfo and 0 or 100)) and
      state.isWebVideo
  ne.tooltip_style = osc_styles.Tooltip
  ne.tooltipF = function()
        ...
        if (not state.videoCantBeDownloaded) then
          ...
          local function openFolder()
            local function is_macos()
              local a = os.getenv("HOME")
              if a ~= nil and string.sub(a, 1, 6) == "/Users" then return true else return false end
            end
