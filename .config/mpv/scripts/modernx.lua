...
local user_opts = {
	...
	title = "${filename/no-ext}", -- title shown on OSC - turn off dynamictitle for this option to apply
...
local osc_styles = {
	TransBg = "{\\blur100\\bord" .. user_opts.OSCfadealpha .. "\\1c&H000000&\\3c&H" .. osc_color_convert(
		user_opts.osc_color
	) .. "&}",
...
function checkcomments()
	...
	local ret = mp.command_native_async({
		...
		local filename = mp.command_native({ "expand-path", user_opts.commentsdownloadpath .. "/" })
			.. mp.get_property("filename"):gsub("watch%?v=", ""):match("^[^%?&]+")
			.. ".info.json"
		print(filename)
...
function exec_description(args, result)
	local ret = mp.command_native_async({
		...
		if
			state.ytdescription == ""
			or state.ytdescription == "\\N"
			or state.ytdescription == "NA"
			or #state.ytdescription < 4
		then
		...
		if beforeLastPattern then
			...
			if desc then
				...
				afterLastPattern = afterLastPattern
					:gsub("Views:", emoticon.view)
					:gsub("Comments:", emoticon.comment)
					:gsub("Likes:", emoticon.like)
					:gsub("Dislikes:", emoticon.dislike) -- replace with icons
...
layouts = function()
	...
	if showinfo then
		...
		lo.geometry = {
			x = osc_geo.w - 172 + (showloop and 0 or 45) + (showontop and 0 or 45),
			y = refY - 40,
			an = 5,
			w = 24,
			h = 24,
	...
	if showscreenshot then
		...
		lo.geometry = {
			x = osc_geo.w - 217 + (showloop and 0 or 45) + (showontop and 0 or 45) + (showinfo and 0 or 45),
			y = refY - 40,
			an = 5,
			w = 24,
			h = 24,
